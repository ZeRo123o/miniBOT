...
class SkillRepository:
    """Persist and query Skill metadata without exposing SQLAlchemy to callers."""
    ...
    async def list_all(self) -> list[Skill]:
        result = await self.db.execute(
            select(Skill).order_by(Skill.updated_at.desc(), Skill.id.desc())
        )
    ...
    async def list_by_slugs(self, slugs: list[str]) -> list[Skill]:
        if not slugs:
            return []
        result = await self.db.execute(select(Skill).where(Skill.slug.in_(slugs)))
        items_by_slug = {item.slug: item for item in result.scalars().all()}
        return [items_by_slug[slug] for slug in slugs if slug in items_by_slug]
