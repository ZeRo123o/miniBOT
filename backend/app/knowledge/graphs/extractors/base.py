...
from app.knowledge.graphs.graph_utils import normalize_entity_name
...
def _normalize_entity(entity: Any, path: str) -> dict[str, Any]:
    ...
    for attribute in attributes:
        ...
        attribute_text = str(attribute.get("text") or "").strip()
        if not attribute_text:
            continue
        normalized_attributes.append(
            {
                "text": attribute_text,
                "label": str(attribute.get("label") or "Attribute").strip()
                or "Attribute",
            }
        )
    return {
