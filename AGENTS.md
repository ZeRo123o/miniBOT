...
- 普通 Tool 在 graph 构建时按 `direct` / `skill_only` / `subagent_only` / `internal` 策略过滤执行域：`direct` 与 `skill_only` 可进入主 Agent 和 Subagent，`subagent_only` 只允许进入 Subagent，`internal` 不进入任何模型 Agent ToolNode。`CapabilityResolver` 区分可执行集合与模型可见集合，`CapabilityMiddleware` 在每次模型调用前过滤 Tool Schema，并在 ToolCall 执行前拒绝不属于可执行集合的调用。Skill 依赖只有在成功读取对应 `SKILL.md` 并激活后才对模型可见，且不得绕过资源启用状态、用户可见范围或 Subagent Profile 白名单。MCP 暂不使用普通 Tool exposure。
...
2. 保存用户消息。
3. 读取用户选择的知识库范围。
4. 读取扩展管理中启用的 MCP、Tool，以及独立 `skills` 表中的 Skill；已启用 Tool/MCP 在 graph 创建时注册为可执行能力，模型仅看到 `CapabilityResolver` 本轮判定可见的工具，Skill 依赖从同一启用资源范围解析。
